...
module FireScratch =
    ...
    type Config() =
        member x.ApiKey with get() = apikey and set(v) = apikey <- v 
        member x.DatabaseURL with get() = databaseURL and set(v) = databaseURL <- v
        member x.DatabasePath with get() = DatabasePath and set(v) = DatabasePath <- v
        member x.Email with get() = email and set(v) = email <- v
        member x.Password with get() = passwd and set(v) = passwd <- v 

    /// FireScratch.config から設定を読み込み
    let LoadConfig() =
        let json = System.IO.File.ReadAllText( configPath )
        let config = Newtonsoft.Json.JsonConvert.DeserializeObject<Config>( json )
        ()
    let SaveConfig() =
        let json = Newtonsoft.Json.JsonConvert.SerializeObject(new Config())
